...
import {ROUND_NAMES, games, choose, fill, statistics, restore, validateField} from './bracket.js';
...
function teamButton(game,team) {
  ...
  return `<button class="team" data-game="${game.id}" data-team="${team.id}" aria-pressed="${selected}" ${game.teams.every(Boolean)?'':'disabled'} aria-label="Pick ${escape(team.name)} in ${ROUND_NAMES[game.round]}"><span class="seed">${team.seed}</span><span class="name">${escape(team.name)}</span><span class="tick" aria-hidden="true">${selected?'✓':''}</span></button>`;
}
function boardHTML(selected) {
  const rounds = games(state.field,state.picks);
...
$('board').addEventListener('click',event=>{
  ...
  if (!button) return;
  const before=Object.keys(state.picks).length;
  state.picks=choose(state.field,state.picks,button.dataset.game,button.dataset.team);
  persist(); render();
  $('board').querySelector(`[data-game="${button.dataset.game}"][data-team="${button.dataset.team}"]`)?.focus();
  const after=Object.keys(state.picks).length;
  announce(after<before ? 'Pick changed. Any incompatible later picks were cleared.' : 'Pick saved. Keep going, or fill the rest.');
});
...
$('next').addEventListener('click',()=>{
  const game=games(state.field,state.picks).flat().find(g=>!g.winner && g.teams.every(Boolean));
  if (!game) { announce('All 63 picks are complete. Export your bracket or review a region.'); return; }
...
$('export').addEventListener('click',()=>{
  ...
  announce('Bracket exported. Import this file later to restore picks, teams, and settings.');
});
$('import').addEventListener('change',async event=>{
  ...
  try{
    ...
    if(Object.keys(state.picks).length && !confirm('Replace the current field and picks with this imported bracket?'))return;
...
$('field-form').addEventListener('submit',event=>{
  ...
  try{
    ...
    if(Object.keys(state.picks).length && !confirm('Use this field and clear the current picks?'))return;
    state.field=field;state.label=$('custom-label').value.trim()||'Custom field';state.picks={};region='South';persist();render();$('field-dialog').close();announce('Your field is ready.');
